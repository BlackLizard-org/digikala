...
def order_details(request, pk):
    if request.user.is_authenticated:
        ...
        messages.success(request, 'دسترسی به این صفحه امکان پذیر نیست.')
...
def user_orders(request):
    if request.user.is_authenticated:
        ...
        context = {'delivered': delivered_orders, 'other': other_orders}
...
def login_user(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            messages.error(request, "مشکلی در ورود کاربر وجود دارد")
...
def update_password(request):
    if request.user.is_authenticated:
        ...
        messages.success(request, 'اول باید وارد بشوید')
        return redirect('home')
